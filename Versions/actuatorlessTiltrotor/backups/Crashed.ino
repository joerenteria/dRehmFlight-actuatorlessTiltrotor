...
float Ki_pitch_angle = 0.4;   //Pitch I-gain - angle mode
float Kd_pitch_angle = 0.05;  //Pitch D-gain - angle mode (if using controlANGLE2(), set to 0.0)
float B_loop_pitch = 0.9;     //Pitch damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)

//Vertical Flight PID
float Kp_roll_angleV = 0.6;    //Roll P-gain - angle mode 
float Ki_roll_angleV = 0.3;    //Roll I-gain - angle mode
//float Kd_roll_angleV = 0.05;
float Kd_roll_angleV = 0.05;   //Roll D-gain - angle mode (if using controlANGLE2(), set to 0.0)
float Kp_pitch_angleV = 0.6;   //Pitch P-gain - angle mode
float Ki_pitch_angleV = 0.3;   //Pitch I-gain - angle mode
//float Kd_pitch_angleV = 0.05;
float Kd_pitch_angleV = 0.05;  //Pitch D-gain - angle mode (if using controlANGLE2(), set to 0.0)
...
//Vertical Yaw
//float Kp_yawV = 0.0;           //Yaw P-gain
//float Ki_yawV = 0.0;          //Yaw I-gain
//float Kd_yawV = 0.00000;       //Yaw D-gain (be careful when increasing too high, motors will begin to overheat!)
...
void loop() {
  ...
  //printModeData();      //prints flight mode status
...
void controlANGLE() {
  ...
  pitch_PID = .01*(Kp_pitch_angle*error_pitch + Ki_pitch_angle*integral_pitch - Kd_pitch_angle*derivative_pitch); //scaled by .01 to bring within -1 to 1 range
    ...
    if (channel_6_pwm > 1500) { 
      ...
      integral_roll = constrain(integral_roll, -i_limit, i_limit); //saturate integrator to prevent unsafe buildup
      derivative_roll = GyroX;
      roll_PID = 0.01*(Kp_roll_angleV*error_roll + Ki_roll_angleV*integral_roll - Kd_roll_angleV*derivative_roll); //scaled by .01 to bring within -1 to 1 range
      ...
      integral_pitch = constrain(integral_pitch, -i_limit, i_limit); //saturate integrator to prevent unsafe buildup
      derivative_pitch = GyroY;
      pitch_PID = .01*(Kp_pitch_angleV*error_pitch + Ki_pitch_angleV*integral_pitch - Kd_pitch_angleV*derivative_pitch); //scaled by .01 to bring within -1 to 1 range
...
void controlMixer() {
  ...
  //Horizontal Flight
  if (channel_6_pwm < 1500) {
  m1_command_scaled = thro_des - pitch_PID  - yaw_PID;
  m2_command_scaled = thro_des - pitch_PID  + yaw_PID;
  m3_command_scaled = thro_des + pitch_PID  + yaw_PID;
  m4_command_scaled = thro_des + pitch_PID  - yaw_PID;
...
void scaleCommands() {
  ...
  //give or take 62, 125-187...

  m1_command_PWM = constrain(m1_command_PWM, 125, 187);
  m2_command_PWM = constrain(m2_command_PWM, 125, 187);
  m3_command_PWM = constrain(m3_command_PWM, 125, 187);
  m4_command_PWM = constrain(m4_command_PWM, 125, 187);
...
void mode() {
    ...
    if (channel_5_pwm < 1500 && channel_6_pwm > 1500) {
    ...
    //Constrain commands to motors within oneshot125 bounds
    //give or take 62, 188-250...
    
    m1_command_PWM = constrain(m1_command_PWM, 188, 250);
    m2_command_PWM = constrain(m2_command_PWM, 188, 250);
    m3_command_PWM = constrain(m3_command_PWM, 188, 250);
    m4_command_PWM = constrain(m4_command_PWM, 188, 250);

}
    else if (channel_5_pwm > 1500 && channel_6_pwm > 1500) {
      m1_command_PWM = 188;
      m2_command_PWM = 188;
      m3_command_PWM = 188;
      m4_command_PWM = 188;
    ...
    if (channel_1_pwm < 1150 && channel_6_pwm > 1500) {
      m1_command_PWM = 188;
      m2_command_PWM = 188;
      m3_command_PWM = 188;
      m4_command_PWM = 188;
