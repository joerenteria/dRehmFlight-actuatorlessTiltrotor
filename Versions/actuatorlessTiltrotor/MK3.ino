...
float B_loop_pitch = 0.9;  //Pitch damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)float Kp_roll_angle = 0.2;    //Roll P-gain - angle mode
...
void loop() {
  ...
  printFlightMode();  //radio pwm values (expected: 1000 to 2000)
  //printverIdle();
  //printhorIdle();
  ...
  controlANGLE();  //stabilize on angle setpoint - Horizontal Flight
  ...
  horizontalMode();
  transitionalMode();
  transitionalToVerticalMode();
  verticalToTransitionalMode();
  verticalMode();
  stoppedMode();
}
...
void controlANGLE() {
  ...
  if (channel_1_pwm < 1060 || flightMode == 1) {  //don't let integrator build if throttle is too low
    integral_roll = 0;
  }
...
void controlMixer() {
  ...
  if (flightMode == 6) {
    ...
    m3_command_scaled = thro_des + yaw_PID;
    m4_command_scaled = thro_des - yaw_PID;
  }

  //--------------------------------------------------
  // Transitional mix
  //--------------------------------------------------
  if (flightMode == 3 || flightMode == 5) {
    m1_command_scaled = 0.5 * thro_des - pitch_PID + roll_PID;
    m2_command_scaled = 0.7 * thro_des - pitch_PID - roll_PID;
    m3_command_scaled = thro_des + yaw_PID;
    m4_command_scaled = thro_des + pitch_PID + roll_PID;
  }

  //4 Transitional to vertical
  //3 Vertical to transitional

  //--------------------------------------------------
  // Vertical mix
  //--------------------------------------------------
  if (flightMode == 2 || flightMode == 4) {
...
void flightModeSelect() {
  //6 Horizontal
  if (channel_6_pwm > 1000 && channel_6_pwm < 1200) {
    if (flightMode == 1 || flightMode == 2 || flightMode == 3 || flightMode == 4 || flightMode == 5) {
      startTime = millis();
      flightMode = 6;
    }
  }
  ...
  //5 Transitional
  if (channel_6_pwm > 1400 && channel_6_pwm < 1600) {

    if (flightMode == 6 || flightMode == 1) {
      startTime = millis();
      flightMode = 5;
    }

    if ((flightMode == 2 || flightMode == 4) && cycleComp == 1) {
      startTime = millis();
      flightMode = 3;
    }
  }
  ...
  //2 Vertical
  if (channel_6_pwm > 1800 && channel_6_pwm < 2000) {

    if ((flightMode == 5 || flightMode == 3) && cycleComp == 1) {
      startTime = millis();
      flightMode = 4;
    }

    if (flightMode == 1 || flightMode == 6) {
      startTime = millis();
      flightMode = 2;
    }
  }
...
// Flight modes
//----------------------------------------------------------------------------------------------------
//Give or take 62. 125 - 187 | 188 - 250
//--------------------------------------------------
// 6 Horizontal
//--------------------------------------------------
...
void horizontalMode() {
  if (flightMode == 6) {
    if (millis() - startTime > period1 && millis() - startTime < period2) {
      m1_command_PWM = horStop;
      m2_command_PWM = horStop;
      m3_command_PWM = horStop;
      m4_command_PWM = horStop;
      //Serial.println("Horizontal Stop...");
    }

    if (millis() - startTime > period2 && millis() - startTime < period3) {
      m1_command_PWM = horStart;
      m2_command_PWM = horStart;
      m3_command_PWM = horStart;
      m4_command_PWM = horStart;
      //Serial.println("Horizontal Start...");
    }

    if (millis() - startTime > period3 && millis() - startTime < period4) {
      m1_command_PWM = horIdle;
      m2_command_PWM = horIdle;
      m3_command_PWM = horIdle;
      m4_command_PWM = horIdle;
      //Serial.println("Horizontal Idle...");
    }

    if (millis() - startTime > period4) {
      m1_command_PWM = 187 - m1_command_scaled * 62;
      m2_command_PWM = 187 - m2_command_scaled * 62;
      m3_command_PWM = 187 - m3_command_scaled * 62;
      m4_command_PWM = 187 - m4_command_scaled * 62;

      m1_command_PWM = constrain(m1_command_PWM, 125, horIdle);
      m2_command_PWM = constrain(m2_command_PWM, 125, horIdle);
      m3_command_PWM = constrain(m3_command_PWM, 125, horIdle);
      m4_command_PWM = constrain(m4_command_PWM, 125, horIdle);
      //Serial.println("Horizontal Run...");
    }
  }
}
...
// 5 Transitional
//--------------------------------------------------
void transitionalMode() {
  if (flightMode == 5) {

    if (millis() - startTime > period1 && millis() - startTime < period2) {
      m1_command_PWM = verStop;
      m2_command_PWM = verStop;
      m3_command_PWM = horStop;
      m4_command_PWM = verStop;

      cycleComp = 0;
      //Serial.println("Transitional Stop...");
    }

    if (millis() - startTime > period2 && millis() - startTime < period3) {
      m1_command_PWM = verStart;
      m2_command_PWM = verStart;
      m3_command_PWM = horStart;
      m4_command_PWM = verStart;
      //Serial.println("Transitional Start...");
    }

    if (millis() - startTime > period3 && millis() - startTime < period4) {
      m1_command_PWM = verIdle;
      m2_command_PWM = verIdle;
      m3_command_PWM = horIdle;
      m4_command_PWM = verIdle;
      //Serial.println("Transitional Idle...");
    }

    if (millis() - startTime > period4) {
      m1_command_PWM = m1_command_scaled * 62 + 188;
      m2_command_PWM = m2_command_scaled * 62 + 188;
      m3_command_PWM = 187 - m3_command_scaled * 62;
      m4_command_PWM = m4_command_scaled * 62 + 188;

      m1_command_PWM = constrain(m1_command_PWM, verIdle, 250);
      m2_command_PWM = constrain(m2_command_PWM, verIdle, 250);
      m3_command_PWM = constrain(m3_command_PWM, 125, horIdle);
      m4_command_PWM = constrain(m4_command_PWM, verIdle, 250);

      cycleComp = 1;
      //Serial.println("Transitional Run...");
    }
  }
}
...
// 4 Transitional to vertical
//--------------------------------------------------
void transitionalToVerticalMode() {
  if (flightMode == 4) {
    if (millis() - startTime > period1 && millis() - startTime < period2) {
      m1_command_PWM = m1_command_scaled * 62 + 188;
      m2_command_PWM = m2_command_scaled * 62 + 188;
      m3_command_PWM = verStop;
      m4_command_PWM = m4_command_scaled * 62 + 188;
      ...
      cycleComp = 0;
      //Serial.println("Transitional-Vertical Stop...");
    }

    if (millis() - startTime > period2 && millis() - startTime < period3) {
      m1_command_PWM = m1_command_scaled * 62 + 188;
      m2_command_PWM = m2_command_scaled * 62 + 188;
      m3_command_PWM = verStart;
      m4_command_PWM = m4_command_scaled * 62 + 188;

      m1_command_PWM = constrain(m1_command_PWM, verIdle, 250);
      m2_command_PWM = constrain(m2_command_PWM, verIdle, 250);
      m4_command_PWM = constrain(m4_command_PWM, verIdle, 250);
      //Serial.println("Transitional-Vertical Start...");
    }

    if (millis() - startTime > period3 && millis() - startTime < period4) {
      m1_command_PWM = m1_command_scaled * 62 + 188;
      m2_command_PWM = m2_command_scaled * 62 + 188;
      m3_command_PWM = verIdle;
      m4_command_PWM = m4_command_scaled * 62 + 188;

      m1_command_PWM = constrain(m1_command_PWM, verIdle, 250);
      m2_command_PWM = constrain(m2_command_PWM, verIdle, 250);
      m4_command_PWM = constrain(m4_command_PWM, verIdle, 250);
      //Serial.println("Transitional-Vertical Idle...");
    }

    if (millis() - startTime > period4) {
      m1_command_PWM = m1_command_scaled * 62 + 188;
      m2_command_PWM = m2_command_scaled * 62 + 188;
      m3_command_PWM = m3_command_scaled * 62 + 188;
      m4_command_PWM = m4_command_scaled * 62 + 188;

      m1_command_PWM = constrain(m1_command_PWM, verIdle, 250);
      m2_command_PWM = constrain(m2_command_PWM, verIdle, 250);
      m3_command_PWM = constrain(m3_command_PWM, verIdle, 250);
      m4_command_PWM = constrain(m4_command_PWM, verIdle, 250);

      cycleComp = 1;
      //Serial.println("Transitional-Vertical Run...");
    }
  }
}

//--------------------------------------------------
// 3 Vertical to transition
//--------------------------------------------------
void verticalToTransitionalMode() {
  if (flightMode == 3) {
    ...
    if (millis() - startTime > period1 && millis() - startTime < period2) {
      ...
      cycleComp = 0;
      //Serial.println("Vertical-Transitional Stop...");
    }
    ...
    if (millis() - startTime > period2 && millis() - startTime < period3) {
      ...
      m2_command_PWM = constrain(m2_command_PWM, verIdle, 250);
      m4_command_PWM = constrain(m4_command_PWM, verIdle, 250);
      //Serial.println("Vertical-Transitional Start...");
    }
    ...
    if (millis() - startTime > period3 && millis() - startTime < period4) {
      ...
      m2_command_PWM = constrain(m2_command_PWM, verIdle, 250);
      m4_command_PWM = constrain(m4_command_PWM, verIdle, 250);
      //Serial.println("Vertical-Transitional Idle...");
    }
    ...
    if (millis() - startTime > period4) {
      ...
      cycleComp = 1;
      //Serial.println("Vertical-Transitional Run...");
    }
  }
}
